...
public static class DependencyInjection
{
    ...
    private static void RegisterLiftdeskClient(IServiceCollection services)
    {
        ...
        // Liftdesk company subscription-plan API — base URL + Bearer key supplied per call from the
        // Liftdesk Project row (customer detail "Paket" tab).
        services
            .AddHttpClient<ILiftdeskPlanClient, LiftdeskPlanClient>(client =>
            {
                client.Timeout = TimeSpan.FromSeconds(60);
            })
            .AddPolicyHandler(BuildCircuitBreakerPolicy());
